...
<%
	...
	List<Library> libraries = mapper.getLibraries();
...
<body>
    ...
    <div class="post-form-container">
        ...
        <form id="post-form" 
        	  method="post"
        	  action="lib-post-modify.jsp?postCateNo=<%=postCateNo %>&postNo=<%=postNo %>">
            <div class="mb-4">
...
%>					 					 
			...
			<div class="mb-4 d" id="selected-lib-container">
				<input type="hidden" name="libNo" id="inquiry-lib" value="<%= post.getLibrary().getNo() %>">
				<label for="title" class="form-label required">문의 도서관</label>
				<div id="selected-lib">
					<!-- 문의 도서관란이 나옴. -->
					<div class="row">
	        			<div class="col-2">
	                		<img style="max-width:100%;" src="<%=post.getLibrary().getImgPath() %>" alt="도서관 표지"/>
	        			</div>
	        			<div class="col-10">
	                		<p><%=post.getLibrary().getName()  %></p>                			
	                		<p><%=post.getLibrary().getLocation() %></p>
	                		<p><%=post.getLibrary().getTel()  %></p>                			
	                		<p><%=post.getLibrary().getBusinessHours() %></p>                			
	        			</div>
	        		</div>
				</div>
			</div>            
